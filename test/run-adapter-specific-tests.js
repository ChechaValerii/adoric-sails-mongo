    ...
    describe('Without associations', () => {
        ...
        describe('Updating a single record', () => {
            it('should update the record correctly', (done) => {
                models.user._adapter.datastores.test.manager.collection('user').insert({ _id: 123, name: 'bob' }, (err) => {
                    if (err) { return done(err); }
                    models.user.update({ id: 123 }, { name: 'joe' }).exec((err, records) => {
                        if (err) { return done(err); }
                        assert.equal(records[0].id, 123);
        ...
        describe('Updating multiple records', () => {
            it('should update the records correctly', (done) => {
                models.user._adapter.datastores.test.manager.collection('user').insert([{ _id: 123, name: 'sid' }, { _id: 555, name: 'nancy' }], (err) => {
                    if (err) { return done(err); }
                ...
                setup(
                    ...
                    (err) => {
                        ...
                        models.pet.create({ name: 'alice' }).exec((err, pet) => {
                            if (err) { return done(err); }
                            petId = pet.id;
                            models.user.create({ id: 123, name: 'scott', friends: [petId] }).exec((err) => {
                                if (err) { return done(err); }
                                return done();
                            });
                        });
...
function setup(fixtures, modelsContainer, cb) {
    ...
    const datastores = {
        test: {
            adapter: 'sails-mongo',
            url: process.env.WATERLINE_ADAPTER_TESTS_URL || 'localhost:27017/sails_mongo',
        },
    };
