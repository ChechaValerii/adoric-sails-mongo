...
const qs = require('qs');
const normalizeDatabase = require('./private/normalize-database');
const normalizeUser = require('./private/normalize-user');
...
module.exports = function normalizeDatastoreConfig(dsConfig, whitelist, expectedProtocolPrefix) {
    ...
    const hasPasswordOverride = !_.isUndefined(dsConfig.password);
    ...
    _.each(Object.keys(dsConfig), (key) => {
    ...
    // FUTURE: Appropriately URL/URIComponent-encode this stuff as we build the url
        ...
        let inventedUrl = `${expectedProtocolPrefix || 'db'}://`;
        ...
        if (parsedConnectionStr.auth && _.isString(parsedConnectionStr.auth)) {
            ...
            if (authPieces[0]) {
                [userInUrl] = authPieces;
            }
            if (authPieces[1]) {
                [, passwordInUrl] = authPieces;
            }
        }

        // Parse the rest of the standard information from the URL.
        const hostInUrl = parsedConnectionStr.hostname;
        const portInUrl = parsedConnectionStr.port;
        let databaseInUrl = parsedConnectionStr.pathname;
        ...
        const newQs = qs.stringify(miscOptsInUrlQs);
        if (newQs.length > 0) {
            rebuiltUrl += `?${newQs}`;
        }
