...
const normalizeMongoObjectId = require('./normalize-mongo-object-id');
...
module.exports = function buildMongoWhereClause(whereClause, WLModel, meta) {
    ...
    const mongoQueryFilter = (function recurse(branch) {
        ...
        if (loneKey === 'and' || loneKey === 'or') {
            const conjunctsOrDisjuncts = branch[loneKey];
            branch[`$${loneKey}`] = _.map(conjunctsOrDisjuncts, (conjunctOrDisjunct) => recurse(conjunctOrDisjunct));
        ...
        const constraintColumnName = loneKey;
        const constraint = branch[constraintColumnName];
        ...
        let doCompareAsObjectIdIfPossible;
        ...
        if (constraintColumnName === pkColumnName && (!meta || !meta.modelsNotUsingObjectIds || !_.contains(meta.modelsNotUsingObjectIds, WLModel.identity))) {
            ...
            _.each(WLModel.attributes, (attrDef /* , attrName */) => {
        ...
        //  ║  ║ ║║║║╠═╝║  ║╣ ╔╩╦╝  ║  ║ ║║║║╚═╗ ║ ╠╦╝╠═╣║║║║ ║
        //  ╚═╝╚═╝╩ ╩╩  ╩═╝╚═╝╩ ╚═  ╚═╝╚═╝╝╚╝╚═╝ ╩ ╩╚═╩ ╩╩╝╚╝ ╩
        const modifierKind = _.first(_.keys(constraint));
        let modifier = constraint[modifierKind];
