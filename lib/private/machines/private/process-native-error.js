...
module.exports = function processNativeError(err) {
    ...
    if (!_.isUndefined(err.footprint)) {
        return new Error(
            ...
            `Consistency violation: Raw error from MongoDB arrived with a pre-existing \`footprint\` property!  Should never happen... but maybe this error didn't actually come from Mongo..?  Here's the error:\n\n\`\`\`\n${err.stack}\n\`\`\`\n`,
        );
    }
