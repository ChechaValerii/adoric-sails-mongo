...
module.exports = {
    ...
    // Dependencies
        ...
        //  ═╩╝╚═╝ ╩ ╚═╝╩╚═╩ ╩╩╝╚╝╚═╝  └┴┘┴ ┴└─┘ ┴ ┴ ┴└─┘┴└─   ┴ └─┘  ╚  ╚═╝ ╩ ╚═╝╩ ╩  └─┘┴└─  ┘└┘└─┘ ┴
        let isFetchEnabled;
        if (s3q.meta && s3q.meta.fetch) { isFetchEnabled = true; } else { isFetchEnabled = false; }
        ...
        (function findMatchingIdsMaybe(proceed) {
            ...
            mongoCollection.find(mongoWhere, projection).toArray((err, nativeResult) => {
                if (err) { return proceed(err); }
                return proceed(undefined, _.pluck(nativeResult, pkColumnName));
            });
        }((err, pkValsOfMatchingRecords) => {
            ...
            mongoCollection.updateMany(secondaryMongoWhere, { $set: s3q.valuesToSet }, (err) => {
                if (err) {
                ...
                if (!_.isUndefined(s3q.valuesToSet[pkColumnName])) {
                    ...
                    if (pkValsOfMatchingRecords.length === 0) { /* do nothing */ } else if (pkValsOfMatchingRecords.length === 1) {
                ...
                mongoCollection.find(secondaryMongoWhere).toArray((err, phRecords) => {
                    if (err) { return exits.error(err); }
