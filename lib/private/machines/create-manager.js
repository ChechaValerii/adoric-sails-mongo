...
const MongodbUriParser = require('./private/mongo-url-parse');
...
module.exports = {
    ...
    fn(inputs, exits) {
        ...
        const NodeMongoDBNativeLib = require('mongodb');
        const CONFIG_WHITELIST = require('../constants/config-whitelist.constant');
        const EXPECTED_URL_PROTOCOL_PFX = require('../constants/expected-url-protocol-pfx.constant');
        const normalizeDatastoreConfig = require('../normalize-datastore-config');
        ...
        const mongoUrl = _clientConfig.url;
        const parsedURL = MongodbUriParser.parse(mongoUrl);
        const hosts = parsedURL.hosts.map((data) => `${data.host}:${data.port}`).join();
        const options = parsedURL.options ? querystring.stringify(parsedURL.options) : '';

        _clientConfig = _.omit(_clientConfig, ['url', 'user', 'password', 'host', 'port', 'database']);

        NodeMongoDBNativeLib.MongoClient.connect(
            `${parsedURL.scheme}://${hosts}/${parsedURL.database}?${options}`,
            _clientConfig,
            (err, client) => {
                ...
                const manager = client.db(parsedURL.database);
