...
const verifyModelDef = require('./private/machines/verify-model-def');
const createManager = require('./private/machines/create-manager');
const destroyManager = require('./private/machines/destroy-manager');
const getConnection = require('./private/machines/get-connection');
const releaseConnection = require('./private/machines/release-connection');
const definePhysicalModel = require('./private/machines/define-physical-model');
const dropPhysicalModel = require('./private/machines/drop-physical-model');
const setPhysicalSequence = require('./private/machines/set-physical-sequence');

const normalizeDatastoreConfig = require('./private/normalize-datastore-config');
...
module.exports = {
    ...
    // Grab the unique name for this datastore for easy access below.
        ...
        try {
            ...
            switch (e.code) {
            case 'E_BAD_CONFIG':
                return done(flaverr(e.code, new Error(`Invalid configuration for datastore \`${datastoreName}\`:  ${e.message}`)));
            default:
                return done(e);
            }
        }
        ...
        if (WET_MACHINES.verifyModelDef) {
            ...
            try {
                _.each(physicalModelsReport, (phModelInfo) => {
                    try {
                        WET_MACHINES.verifyModelDef({
                            modelDef: phModelInfo,
                        }).execSync();
                    } catch (e) {
                        switch (e.exit) {
                        case 'invalid':
    ...
    // Look up the datastore entry (manager/driver/config).
        ...
        //  ═╩╝╚═╝╚═╝ ╩ ╩╚═╚═╝ ╩   ┴ ┴┴ ┴┘└┘┴ ┴└─┘└─┘┴└─
        // Destroy the manager.
        WET_MACHINES.destroyManager({
            manager: dsEntry.manager,
        }).switch({
            error(err) {
                return done(new Error(`Encountered unexpected error when attempting to destroy the connection manager.\n\n\`\`\`\n${err.stack}\n\`\`\``));
            },
            failed(report) {
            ...
            success(report) {
                ...
                try {
                    ...
                    _.each(_.keys(registeredDryModels), (modelIdentity) => {
    ...
    // -----------------------------------------------------
    // N/A
    // (@adoric/sails-mongo does not implement an optimized `join`
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     */
    define(datastoreName, tableName, phmDef, done) {
    // Look up the datastore entry (manager/driver/config).
        const dsEntry = registeredDsEntries[datastoreName];
        ...
        const uniqueIndexesToCreate = [];
        _.each(phmDef, (phmAttrDef, key) => {
        ...
        // > Remove `_id`.
        _.remove(uniqueIndexesToCreate, (val) => val === '_id');
        ...
        async.each(uniqueIndexesToCreate, (key, next) => {
            ...
            mongoCollection.createIndex(mongoSingleFieldIdxKeys, {
                unique: true,
            }, (err) => {
    ...
    // Look up the datastore entry (manager/driver/config).
        ...
        const db = dsEntry.manager;
        db.collection(tableName).drop((err) => {
            try {
                if (err) {
    ...
    // -----------------------------------------------------
    // N/A
    // (@adoric/sails-mongo does not currently implement setSequence.)
    // -----------------------------------------------------
